...
class PuzzleActivityViewModel(
    ...
    private var puzzleRuler: PuzzleRuler? = state.get(ACTIVITY_STATE_PUZZLE_SLN)
    ...
    private fun writeInBD(state: Int, action: () -> Unit) {
        puzzleRuler?.let {
            viewModelScope.launch {
                val dao = getApplication<PuzzleOfDayApplication>().historyDb.getPuzzleHistoryDao()
                val pgn = it.getPgn().reduce { acc, s -> "$acc $s" }
                val sln = if (it.getSln().isNotEmpty())
                    it.getSln().reduce { acc, s -> "$acc $s" }
                else ""
                dao.update(it.id, pgn, sln, state)
                action()
            }
        }
    }
    ...
    override fun onCleared() {
        super.onCleared()
        state.set(ACTIVITY_STATE_PUZZLE_BOARD, puzzleBoard)
        state.set(ACTIVITY_STATE_PUZZLE_SLN, puzzleRuler)
    }
}